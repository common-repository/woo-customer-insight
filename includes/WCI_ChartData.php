...
class WCI_ChartData {
	...
	{
        	$wci_failed = '';
                $wci_failed .= "<table class='table table-hover' style='width:99%;border:1px solid #E5E5E5;'>
                <thead>
...
{
       		$success_payment = '';
                $success_payment .= "<table class='table table-hover' style='width:99%;border:1px solid #E5E5E5;'>
                <thead>
                <tr class='thone'>
                <th class='col-md-3'> Customer Name</th>
                <th class='col-md-2'> Email</th>
...
{
             $wci_abundant_cart = '';
             $wci_abundant_cart .= "<table class='table table-hover' style='width:99%;border:1px solid #E5E5E5;'>
                <thead>
                <tr class='thone'>
                <th class='col-md-3;'> Customer Name</th>
...
if(empty( $get_session_ids )  )
{
               $wci_addtocart .= "<table class='table table-hover' style='width:99%;border:1px solid #E5E5E5;'>
                <thead>
                <tr class='thone' style='width:100%;'>
                <th class='col-md-3;'> Customer Name</th>
                <th class='col-md-3;'> Country </th>
...
//END Addtocart
	...
	$html = '';
	$html .= "<table class='table table-hover' id='abandon_cart_list' style='width:99%;border:1px solid #E5E5E5;'>";
...
else
{
	...
	public  function wci_funnel_chart()
	{
		global $wpdb;
		$funnel_date = '';
		...
		if( $funnel_date == 'customdate' || $funnel_date == "last_week" || $funnel_date == "last_month" || $funnel_date == "last7days" || $funnel_date == "last30days" || $funnel_date == "" )
		{
			...
			$total_visitors = $wpdb->get_results( "select count( distinct session_id,session_key) as total from wci_activity where date between '$from_date' and '$to_date'" );
			...
			else
			{
				$total_visitors_count = 0 ;
                	...
                	$AddToCart = $wpdb->get_results("select count(distinct session_id) as event1 from wci_user_session where date between '$from_date' and '$to_date'");
			if( !empty( $AddToCart ))
			{
			...
			else
			{
				$Checkout = 0;
                	...
                	$successful_purchase = $wpdb->get_results( "select count(distinct session_id) as total_purchase from wci_user_session where date between '$from_date' and '$to_date' and payment_success='1'",ARRAY_A );
...
$most_visited_spent_time = $wpdb->get_results( "SELECT information from wci_activity where date between '$from_date' and '$to_date' and clicked_button in('Page visit','AddToCart')",ARRAY_A );
...
$visit_pages = $wpdb->get_results($wpdb->prepare("select information from wci_activity where clicked_button in(%s,%s) and date between '$from_date' and '$to_date'" , 'Page visit','AddToCart' ),ARRAY_A );
...
$most_visit[$value['visited_url']][]=$value['spent_time'];
        ...
        $most_spent_time = array();
$most_visited_spent_time = $wpdb->get_results( "SELECT * from wci_activity where date like '$funnel_date' and clicked_button in('Page visit','AddToCart')",ARRAY_A );
        foreach( $most_visited_spent_time as $info_key => $info_vals )
        {       $k++;
                $unserialized_spent_array = unserialize( $info_vals['information'] );
                $most_spent_time[$k]['prod_id'] = $unserialized_spent_array['prodid'];
...
$visit_pages = $wpdb->get_results( "select information from wci_activity where date like '$funnel_date' and clicked_button in('Page Visit','AddToCart')",ARRAY_A );
...
{
$order_summary = "<table class='table table-hover' style='width:100%;border:1px solid #E5E5E5;'>";
$order_summary .= "<tr class='thone'><th class='col-md-2' style='text-align:left;'>Order Id </th><th class='col-md-1'></th><th class='col-md-4'>Order Holder ID</th></th>
<th class='col-md-4' style='text-align:right;'>Total(in ".$currency." )</th><th class='col-md-1'></th></tr>";
...
//			$funnel_date =  $_POST['date'];
	...
	public function wci_get_pie_data(){
		global $wpdb;
		$target_person = $_POST['selected_customer'];
		$target_user_id = $_POST['selected_user_id'];
		$target_visited = $wpdb->get_results($wpdb->prepare("select count(*) as visited from wci_activity where user_id=%s and clicked_button in(%s,%s,%s)", $target_person , 'AddToCart','Checkout','Page visit'));
		...
		if($sum_value == 0) {
			...
			$i++;
				...
				$html .= '<td>' .$info_value->clicked_button. '</td>';
...
$get_order_count = $wpdb->get_results( $wpdb->prepare( "select count(*) as total_order from wci_successful_purchases where user_email=%s and user_name=%s" , $wci_user_email , $target_person ) );
$wci_order_count = $get_order_count[0]->total_order;

$wci_currency_symbol = get_woocommerce_currency_symbol();

$get_total_money_spent = $wpdb->get_results( $wpdb->prepare( "select sum(total_price) as total_price from wci_successful_purchases where user_email=%s and user_name=%s and order_status=%s" , $wci_user_email , $target_person , 'Completed' ) );
...
$get_success_orders = $wpdb->get_results( $wpdb->prepare( "select count(*) as success_orders from wci_successful_purchases where user_name=%s and user_email=%s and order_status=%s" , $target_person , $wci_user_email, 'Completed' ) );
...
//END GET ALL DATA
	...
	$profile_stats = '';
	$profile_stats .= "<div style='width:70%;height:100%;background-color: #FFFFFF;border-radius:3px;margin-left:12%;border:1px solid #D3D3D3;' id='profile'>";//PROFILE

        $profile_stats .= "<br><div id ='one' style='width:100%;height:25%;'>";
        $profile_stats .= "<div id='avatar'>". get_avatar( $target_user_id , 100 ). " </div> <div id='stat_user_name' style='font-weight:bold;'> ".ucwords($target_person)." <br><br>  ". $wci_location."</div>";

	$profile_stats .= "<div id='last_login' style='color:#0095D4;font-size:12px;width:100%;float:left;margin-left:130px;'> Last login :".$wci_last_login." </div>";
        $profile_stats .= "</div>";
        //EMAIL
