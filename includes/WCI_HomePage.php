...
<!-- WOO CUSTOMER INSIGHT HOME PAGE -->

<style>
html,body
{
	margin:0; background-color:#FFFFFF; height:100px; 
}
select {
        width:120px;
    }
...
echo "<html><body >"; 
//Whole div
echo "<div id='dashboard_container' style='width:100%;font-family: Verdana,Geneva,sans-serif;'>";
echo "<br><div id='chart_title'>" .WCI_TITLE. " :: Dashboard </div>";
...
echo '<th class="col-md-2" style="text-align:right;"> Choose Filter</th>';
echo '<th class="col-md-2"><select id="filter_combo"  name ="filter_comb">
...
</select></th></tr><tr><br></tr>
<tr id="custom_datepicker">
<td></td>
<td></td>
<td></td>
<td><input type="text" id="custom_datepicker_start" class="datepicker" style="width:190px;" placeholder="Start Date" ></td>
<td><input type="text" id="custom_datepicker_end" class="datepicker" style="width:190px;margin-left:2%" placeholder="End Date"> </td>
<td><input type="button" class="button button-primary" id="custom_dates" value="Go"></td>
</tr>
</table>';
...
echo "<div id='wootracking_chart_container' style='min-width: 10%; max-width: 55%; height: 40%; margin-left:21%'>
</div></div>";
$sample_chart =  WP_PLUGIN_URL . "/woo-customer-insight/images/Sample.png";
...
<div  style='background-color:#E0E6E9;width:99%;margin-bottom:0px;'>
      <div class='tracked_info' style='width:99%;height:48px;'>
	<span style='font-family:sans-serif;padding-left:1%;float:left;height:100%;'> <h4> <?php echo esc_html("Tracked Events Info"); ?>:</h4> </span>
        
        <div class='set_line_height' style='margin-left:55%;'>
        <span class="add_highlight " id='tab_login' style=''>

        <a id="filter_login" style='color:#000000 !important;text-decoration:none;'> <?php echo esc_html("Login") ; ?></a> <span> | </span> 
        </span>
        <span class="add_highlight" id='tab_purchase' style=''>
                <a id='filter_cart' style='color:#000000 !important;text-decoration:none;'><?php echo esc_html("User Purchases");?></a> <span> | </span>
        </span>

         <span class="add_highlight" id='tab_abundant' style=''>
                <a id='filter_abundant' style='color:#000000 !important;text-decoration:none;'><?php echo esc_html("Abandon Orders");?></a>
        </span>
        </div>
      </div>
</div>
...
global $wpdb;
$user_query_string = admin_url()."admin.php?page=customer_logs&user_id=";
$login_time = array();
...
echo "<table class='table table-hover common_hide_area' id='user_login' style='display:none;width:99%;border:1px solid #E5E5E5'>";
echo "<tr class='thone'>
...
echo "<table class='table table-hover common_hide_area' id='user_purchased'  style='display:none;width:99%;border:1px solid #E5E5E5;'>";
...
echo "<table class='table' id='user_purchased' style='display:none;width:99%;'>";
...
echo '<br>
<table id="abundant_orders_table" style="width:100%;"><td class="col-md-2">
...
<td>
<input type="button" id="abandon_submit" style="display:none;"  name="abandon_submit" class="btn btn-primary" value="Fetch Data" />
</td>
<td class="col-md-6"> </td>
<td>
<a href="" id="abundant_oneday" style="text-align:right;">1D </a><span style="padding-left:10px;"> | </span> </td>
<td>
<a href="" id="abundant_oneweek"  style="text-align:right;">1W </a> <span style="padding-left:10px;"> | </span></td>
<td>
<a href="" id="abundant_onemonth" style="text-align:left;">1M </a></td>
</tr></table>';
echo '<br>';
...
echo "<table class='table' id='abandon_cart_list' style='width:100%'>";
echo "<tr class='thone'><th class='col-md-4'>Customer Email</th><th class='col-md-4'>Order ID</th><th class='col-md-4'>Order Date</th></tr>";
echo "<tr class='rowone'><th></th><th>-- No Records Found --</th><th></th></tr></table>";
}
...
$post_title = $wpdb->get_results($wpdb->prepare("select post_title from ".$wpdb->prefix."posts where post_type=%s", 'product'));
	...
	echo "<br>";
	echo "<div id='dashboard_title_three' style='width:99%;'>";
	echo "<div id='table_cell_one'>";
	echo "<div class='header_title_widget' ><h4>".esc_html('Most sold products'). ": </h4></div>";
...
{
	$most_sold = '';
	$most_sold .= "<table class='table table-hover' style='width:100%;border:1px solid #E5E5E5;'>";
...
$info_most_visited = $serialized_most_visited = $url_list = $url_with_count = $url_with_count1 = array();
$info_most_visited = $wpdb->get_results("SELECT * FROM wci_activity",ARRAY_A); 

foreach($info_most_visited as $most_visited_key=> $most_visited_value)
{  
$serialized_most_visited = unserialize($most_visited_value['information']); 
$url_list[] = $serialized_most_visited['page'];    
}
$url_with_count1 =  array_count_values($url_list); 
arsort($url_with_count1);    
$url_with_count = array_slice($url_with_count1,0,5);
...
foreach( $visit_page as $value ){
	if( preg_match("/product/",$value['prod_pageurl'] ) )
	{
		...
		$product_name[] = $get_prod_from_db[0]['post_title'] ;
...
<div class='header_title_widget' ><h4> <?php echo esc_html("Most Visited products-No of Visits") ;?></h4></div>
<div id='product_no_of_visit'>
<?php
if(empty($product_name))
{

$prod_no_of_visit = '<table class="table table-hover" style="width:100%;border:1px solid #E5E5E5;" >
<thead>
<tr class="thone">
...
<table class="table table-hover" style='width:100%;border:1px solid #E5E5E5;'>
<thead>
<tr class="thone">
...
echo "<br>";
echo "<div id='dashboard_title_four' style='width:99%'>";
echo "<br>";
echo "<div id='table_cell_one'>";
echo "<div class='header_title_widget' ><h4>Most Active Customers By Revenue : </h4></div>";
...
echo "<div class='header_title_widget' ><h4>Most Active Customers By Orders : </h4></div>";
...
echo "<div id='table_cell_one'>";
echo "<div class='header_title_widget' ><h4>Most Visited Products-Time Spent: </h4></div>";
...
//Order-summary
echo "<div id='table_cell_two'>";
echo "<div class='header_title_widget' ><h4> ". esc_html('Orders - Summary : ( Recent 5 Orders )')." </h4></div>";
...
echo "</div>";
echo "</div>";
echo "</div>";//END dashboard_container
echo "</body></html>";
?>